...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-black text-white font-mono relative overflow-hidden cyber-bg">
      ...
      <button
        className="sm:hidden absolute top-4 left-4 p-2 bg-gray-800 rounded z-20"
      ...
      <div className="flex flex-1">
        {/* Sidebar */}
        <aside
          className={`fixed sm:relative z-10 h-full top-0 left-0 w-64 sm:w-72 lg:w-80 bg-black border-r border-transparent p-2 overflow-y-auto transform transition-transform duration-300 ${
            sidebarOpen ? "translate-x-0" : "-translate-x-full sm:translate-x-0"
          }`}
        >
          <div className="absolute inset-0 animate-borderGlow pointer-events-none"></div>
          <div className="relative z-10 p-4">
            <h2 className="font-semibold mb-4 text-lg tracking-wider">COMPANIES</h2>
              ...
              {companies.map((sym) => (
                <li
                  ...
                  className={`p-3 rounded cursor-pointer ${
                    selected === sym ? "bg-slate-900 font-bold" : "hover:bg-slate-800"
                  }`}
                >
                  {sym}
                </li>
        ...
        {/* Main Content */}
        <main className="flex-1 p-4 sm:p-6 flex flex-col ml-0 sm:ml-0">
          {/* Chart */}
          <h1 className="text-2xl font-bold mb-4 tracking-wide">
            {selected ? `${selected} STOCK PRICE` : "SELECT A COMPANY"}
          </h1>
          <div className="bg-black border border-gray-800 rounded p-4 shadow-lg animate-glowPulse h-64 sm:h-80 lg:h-96">
            {loading ? (
              ...
              selected && <PriceChart labels={labels} data={data} />
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
            ...
            <div className="bg-gray-900 p-4 rounded shadow-lg animate-glowPulse">
              ...
              {overview ? (
                <>
                  ...
                  <p className={Number(overview.change) >= 0 ? "text-green-400" : "text-red-500"}>
            ...
            <div className="bg-gray-900 p-4 rounded shadow-lg animate-glowPulse">
              ...
              {movers.map((m) => (
                <p key={m.symbol} className={m.change >= 0 ? "text-green-400" : "text-red-500"}>
            ...
            {newsLoading ? (
              ...
              news.map((n, idx) => (
                <a
                  ...
                  className="bg-gray-900 p-4 rounded shadow-lg hover:shadow-xl transition-shadow duration-300 break-words"
                >
                  <h4 className="font-bold text-lg text-blue-400 mb-2 break-words">{n.title}</h4>
      ...
      {/* Animations */}
      <style jsx global>{`
        @keyframes glowPulse {0%,100%{box-shadow:0 0 5px #ffffff22;}50%{box-shadow:0 0 15px #ffffff33;}}
        @keyframes blink {0%,50%,100%{opacity:1;}25%,75%{opacity:0;}}
        @keyframes cursorBlink {0%,50%,100%{opacity:1;}25%,75%{opacity:0;}}
        @keyframes gridMove {0%{background-position:0 0;}100%{background-position:80px 80px;}}
        @keyframes borderGlow {0%{border-color:rgba(255,255,255,0.2);}50%{border-color:rgba(255,255,255,0.6);}100%{border-color:rgba(255,255,255,0.2);}}
        ...
        .animate-gridMove {background-image:linear-gradient(90deg,#ffffff11 1px,transparent 1px),linear-gradient(#ffffff11 1px,transparent 1px);background-size:40px 40px;animation:gridMove 12s linear infinite;}
        .animate-borderGlow {border-right:2px solid;animation:borderGlow 4s infinite;}
